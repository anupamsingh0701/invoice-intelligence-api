...
logger = logging.getLogger("invoice-intelligence-api")

app = FastAPI(
    title="Invoice Intelligence Structured Extraction API",
    description="Extracts structured fields from messy free-text invoices matching a schema using Gemini API.",
    version="1.0.0"
)
...
# Helper functions for parsing & normalization
...
def parse_date_to_iso(text: str) -> Optional[str]:
    ...
    # 2. DD Month YYYY (e.g., 12 June 2026 or 12th June 2026 or 12-June-2026)
    m2 = re.search(r'\b(\d{1,2})(?:st|nd|rd|th)?[-/\s]+([a-zA-Z]+)[-/\s]+(\d{4})\b', text)
    if m2:
        d, m_str, y = m2.group(1), m2.group(2).lower(), m2.group(3)
        if m_str[:3] in [k[:3] for k in MONTHS_MAP]:
            m_num = [MONTHS_MAP[k] for k in MONTHS_MAP if k.startswith(m_str[:3])][0]
...
@app.get("/")
def root():
    return {"status": "ok", "service": "invoice-intelligence-extraction"}
